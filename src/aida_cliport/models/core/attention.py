...
class Attention(nn.Module):
    ...
    def __init__(self, stream_fcn, in_shape, n_rotations, preprocess, cfg, device, verbose=False, n_skills=1):
        ...
        if self.n_skills > 1:
            assert self.n_rotations == 1, "Multiple skills only supported with single rotation."
    ...
    def _build_nets(self):
        ...
        self.attn_stream = models.names[stream_one_fcn](self.in_shape, self.n_skills, self.cfg, self.device)
        if self.verbose:
