...
from aida_cliport.models.core.attention import Attention
import aida_cliport.models as models
import aida_cliport.models.core.fusion as fusion


class TwoStreamAttentionLangFusion(Attention):
    """Two Stream Language-Conditioned Attention (a.k.a Pick) module."""

    def __init__(self, stream_fcn, in_shape, n_rotations, preprocess, cfg, device, verbose=False, n_skills=1):
        self.fusion_type = cfg["train"]["attn_stream_fusion_type"]
        super().__init__(stream_fcn, in_shape, n_rotations, preprocess, cfg, device, verbose=verbose, n_skills=n_skills)
    ...
    def _build_nets(self):
        ...
        self.attn_stream_one = stream_one_model(self.in_shape, self.n_skills, self.cfg, self.device, self.preprocess)
        self.attn_stream_two = stream_two_model(self.in_shape, self.n_skills, self.cfg, self.device, self.preprocess)
    ...
    def forward(self, inp_img, lang_goal, softmax=True):
        ...
        B = in_tens.shape[0]
        if self.n_skills > 1:
            logits = logits.reshape(self.n_skills, B, *logits.shape[2:])  # [S B W H]
        else:
            R = self.n_rotations
...
class TwoStreamAttentionLangFusionLat(TwoStreamAttentionLangFusion):
    ...
    def __init__(self, stream_fcn, in_shape, n_rotations, preprocess, cfg, device, n_skills=1):
        self.fusion_type = cfg["train"]["attn_stream_fusion_type"]
        super().__init__(stream_fcn, in_shape, n_rotations, preprocess, cfg, device, n_skills=n_skills)
