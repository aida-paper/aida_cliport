...
from aida_cliport.tasks import cameras
from aida_cliport.utils import utils
from aida_cliport.models.core.attention import Attention
from aida_cliport.models.streams.two_stream_attention import TwoStreamAttention
from aida_cliport.models.streams.two_stream_transport import TwoStreamTransport

from aida_cliport.models.streams.two_stream_attention import TwoStreamAttentionLat
from aida_cliport.models.streams.two_stream_transport import TwoStreamTransportLat
...
class TransporterAgent(LightningModule):
    def __init__(self, name, cfg, train_ds, test_ds):
        ...
        if "train_interactive" in cfg:
            ...
            self.thrifty = cfg["train_interactive"]["thrifty"]
            self.weight_decay = cfg["train_interactive"]["weight_decay"]
            if self.thrifty:
                assert "dropout" in str(type(self)).lower(), "Thrifty training requires dropout."
                assert int(cfg["train_interactive"]["N"]) > 1, "Thrifty training requires dropout evaluations N > 1."
                assert (
                    float(cfg["train_interactive"]["drop_prob"]) > 0.0
                ), "Thrifty training requires dropout probability drop_prob > 0.0."
        elif "train" in cfg:
