...
import aida_cliport
from aida_cliport import agents
from aida_cliport.dataset import RavensDataset, collate_fn
from aida_cliport.sag import sag
from aida_cliport.utils import utils
from aida_cliport.pier import prioritize_sampling
...
class InteractiveAgent:
    ...
    def prepare_data_dir(self):
        ...
        if self._icfg["tmp_dir"] is not None:
            tmp_dir = str(Path(self._icfg["tmp_dir"]) / "aida")
    ...
    def update_stats(self, demo):
        ...
        self._stats["queries"][-1] = demo["r"] == aida_cliport.KNOWN_SUCCESS or demo["r"] == aida_cliport.KNOWN_FAILURE
        self._stats["demos"].append(demo["demo"] is not None)
        if demo["relabeling_demo"] is not None:
            self._u.append(np.NaN)
            self._r.append(aida_cliport.UNKNOWN_RELABELING)
    ...
    def act(self, obs, info):
        ...
        self._r.append(aida_cliport.UNKNOWN_ONLINE)
        self._k.append(self._stats["n_updates"])
    ...
    def _log_stats(self):
        ...
        n_validation = (
            np.sum(np.logical_and(r == aida_cliport.KNOWN_SUCCESS, np.asarray(demos))) if self._validation_demos else 0
        )
        n_relabeling = np.sum(r == aida_cliport.UNKNOWN_RELABELING)
    ...
    def _initialize_stats(self):
        if len(self._k) == 0 and self._train_ds.n_demos > 0 or not self._save_results:
            ...
            self._r = [aida_cliport.UNKNOWN_OFFLINE] * n_pairs
    ...
    def _get_logger(self):
        ...
        if self._logging:
            ...
            self._wandb_logger = WandbLogger(project="aida-cliport", name=log_name)
