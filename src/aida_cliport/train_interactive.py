...
import aida_cliport
from aida_cliport.interactive_agent import InteractiveAgent
from aida_cliport import tasks
from aida_cliport.utils import utils
from aida_cliport.environments.environment import Environment
...
def collect_demo(
    ...
    demo = dict(
        r=aida_cliport.UNKNOWN_ONLINE, novice_reward=deepcopy(reward), oracle_demo=False, demo=None, relabeling_demo=None
    )

    if np.random.rand() < p_rand or query:
        demo.update(r=aida_cliport.KNOWN_SUCCESS if np.abs(max_reward - reward) < 0.01 else aida_cliport.KNOWN_FAILURE)
